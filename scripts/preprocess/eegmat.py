...
if __name__ == '__main__':
    ...
    dataset.initialize({
        'eeg': {
            'sample_shape': (EPOCH_SECONDS * TARGET_SR, 19,),
            'chunks': (1, EPOCH_SECONDS * TARGET_SR, 19),
            'type': np.float32,
            'compression': None,
        },
        'filename': {
    ...
    for file in tqdm(files):
        ...
        eeg = eeg[:-(eeg.shape[0] % (TARGET_SR * EPOCH_SECONDS))]
        eeg = eeg.reshape(-1, TARGET_SR * EPOCH_SECONDS, 19)

        f = os.path.basename(file)
        thinker = int(f[len('Subject'):][:2])
        label = int(f[len('Subject00_'):][:1]) - 1

        dataset.append({
            'eeg': eeg,
            'filename': file,
            'label': np.ones(eeg.shape[0], dtype=int) * label,
            'thinker': np.ones(eeg.shape[0], dtype=int) * thinker,
        })

    print(dataset.file['eeg'].attrs['len'])
